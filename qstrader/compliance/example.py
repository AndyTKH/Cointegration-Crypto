...
class ExampleCompliance(AbstractCompliance):
    ...
    def __init__(self, config):
        ...
        try:
            fname = os.path.expanduser(os.path.join('/Users/AndyTan/Documents/cointegration_crypto/', self.csv_filename))
            os.remove(fname)
        except (IOError, OSError):
            print("No tradelog files to clean now.")

        # Write new file header
        fieldnames = ["timestamp", "ticker","action", "quantity","exchange", "price","commission"]
        fname = os.path.expanduser(os.path.join('/Users/AndyTan/Documents/cointegration_crypto/', self.csv_filename))
